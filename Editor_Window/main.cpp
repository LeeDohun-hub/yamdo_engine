...
//#pragma comment(lib, "..\\x64\\Debug\\YamdoEngine_Window.lib")
                     ...
                     _In_ int       nCmdShow)
{
    ...
    // 메시지큐에 아무것도 없다면?? 아무 메시지도 가져오지 않게 된다

    // PeekMessage : 메시지큐에 메시지 유무에 상관없이 함수가 리턴된다
    // 리턴값이 true인 경우 메시지가 있고 false인 경우는 메시지가 없다고 가르쳐준다.
